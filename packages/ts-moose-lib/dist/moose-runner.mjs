...
function getCompiledIndexPath(projectRoot2 = process.cwd()) {
  ...
  return path2.resolve(projectRoot2, outDir, sourceDir, "index.js");
}
...
function detectModuleSystem(projectRoot2 = process.cwd()) {
  const pkgPath = path2.join(projectRoot2, "package.json");
...
var MOOSE_COMPILER_PLUGINS, MOOSE_COMPILER_OPTIONS, MOOSE_MODULE_OPTIONS, DEFAULT_OUT_DIR;
...
function findSourceFiles(dir, onReadError) {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      files.push(path3.resolve(fullPath));
    }
  }
  return files;
}
var DEFAULT_SOURCE_EXTENSIONS;
...
function getObjectPropertyExpression(objectLiteral, propertyName) {
  for (const property of objectLiteral.properties) {
    if (ts.isPropertyAssignment(property)) {
      const name = ts.isIdentifier(property.name) ? property.name.text : ts.isStringLiteral(property.name) ? property.name.text : void 0;
...
function resolveSymbolInitializerExpression(declaration) {
  ...
  if ((ts.isPropertyAccessExpression(declaration) || ts.isElementAccessExpression(declaration) || ts.isIdentifier(declaration)) && ts.isBinaryExpression(declaration.parent) && declaration.parent.left === declaration && declaration.parent.operatorToken.kind === ts.SyntaxKind.EqualsToken) {
...
function resolveResourceFromExpression(expression, ctx, bindings) {
  ...
  if (ts.isTaggedTemplateExpression(unwrapped) && isSqlTag(unwrapped.tag, ctx.checker)) {
...
function getFunctionLikeDeclarations(symbol, ctx) {
  ...
  const declarations = [];
  for (const declaration of resolvedSymbol.declarations ?? []) {
    if (ts.isFunctionDeclaration(declaration) || ts.isMethodDeclaration(declaration) || ts.isGetAccessorDeclaration(declaration) || ts.isSetAccessorDeclaration(declaration)) {
...
function isSqlTag(tag, checker) {
  ...
  if (ts.isPropertyAccessExpression(unwrapped) && unwrapped.name.text === "sql") {
    return true;
  }
  const symbol = resolveAliasedSymbol(
...
function isUserCodeFunction(fn) {
  const fileName = path4.resolve(fn.getSourceFile().fileName).replace(/\\/g, "/");
  const cwd = path4.resolve(process2.cwd()).replace(/\\/g, "/");
...
function analyzeFunctionGraph(roots, ctx) {
  ...
  const visitFunction = (fn, bindings) => {
    ...
    const visitNode = (node) => {
      if (ts.isTaggedTemplateExpression(node) && isSqlTag(node.tag, ctx.checker)) {
...
function collectAnalysisFiles(registry) {
  ...
  for (const api of uniqueApis) {
    ...
    if (typeof sourceFile === "string" && fs3.existsSync(sourceFile) && isSourceFilePath(sourceFile)) {
      files.add(path4.resolve(sourceFile));
    } else {
  ...
  if (files.size === 0 || requiresFallbackScan) {
    const appDir = path4.resolve(process2.cwd(), getSourceDir());
...
function loadCompilerOptions(rootNames) {
  ...
  const parsed = ts.parseJsonConfigFileContent(
    configFile.config,
    ts.sys,
    path4.dirname(configPath)
  );
  const normalizedRoots = [
    ...new Set(rootNames.map((file) => path4.resolve(file)))
  ];
  return {
...
var WRITE_METHODS, API_HELPER_IDENTIFIERS;
...
function findUnloadedFiles() {
  ...
  const loadedStems = new Set(
    Object.keys(__require.cache).filter((key) => key.startsWith(compiledAppDir)).map((key) => pathStem(path5.relative(compiledAppDir, key)))
  );
...
var MutationTrackingMap, registryMutationVersion, lineageCache, markRegistryMutated, moose_internal, defaultRetentionPeriod, toInfraMap, initializeMooseInternalRegistry, getMooseInternal, dumpMooseInternal, loadIndex, getStreamingFunctions, getApis2, getWorkflows2, getTaskForWorkflow, getWebApps2;
...
var LoggerSingleton = class _LoggerSingleton {
  ...
  static initializeLogger() {
    if (!_LoggerSingleton.instance) {
      _LoggerSingleton.instance = new DefaultLogger(
        "DEBUG",
        ({ level, message }) => {
...
var packageJson = JSON.parse(
  readFileSync4(join4(__dirname, "..", "package.json"), "utf-8")
);
...
program.parse();
//# sourceMappingURL=moose-runner.mjs.map