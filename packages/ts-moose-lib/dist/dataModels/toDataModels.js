...
var toDataModels_exports = {};
__export(toDataModels_exports, {
  default: () => toDataModels_default
});
module.exports = __toCommonJS(toDataModels_exports);
...
// src/dataModels/enumConvert.ts
var isEnum = (t) => !!(t.getFlags() & import_typescript.default.TypeFlags.EnumLiteral);
...
var dateType = (checker) => checker.getTypeOfSymbol(
  checker.resolveName("Date", void 0, import_typescript2.SymbolFlags.Type, false)
...
var tsTypeToDataType = (t, checker, fieldName, typeName, isJwt, typeNode) => {
  ...
  if (typeNode && (0, import_typescript2.isTypeReferenceNode)(typeNode)) {
    const tn = typeNode.typeName;
    const name = (0, import_typescript2.isIdentifier)(tn) ? tn.text : tn.right.text;
    if (name === "DateTime64") {
      const arg = typeNode.typeArguments?.[0];
      if (arg && import_typescript2.default.isLiteralTypeNode(arg) && import_typescript2.default.isNumericLiteral(arg.literal)) {
  ...
  if (isEnum(nonNull)) {
    ...
    } else if (withoutTags.isClassOrInterface() || (withoutTags.flags & import_typescript2.TypeFlags.Object) !== 0) {
...
var hasWrapping = (typeNode, wrapperName) => {
  if (typeNode !== void 0 && (0, import_typescript2.isTypeReferenceNode)(typeNode)) {
    const typeName = typeNode.typeName;
    const name = (0, import_typescript2.isIdentifier)(typeName) ? typeName.text : typeName.right.text;
...
var handleDefaultWrapping = (typeNode) => {
  if (typeNode !== void 0 && (0, import_typescript2.isTypeReferenceNode)(typeNode)) {
    ...
    if (name === "WithDefault" && typeNode.typeArguments?.length === 2) {
      ...
      if (import_typescript2.default.isLiteralTypeNode(defaultValueType) && import_typescript2.default.isStringLiteral(defaultValueType.literal)) {
...
var toColumns = (t, checker, options) => {
  ...
  return checker.getPropertiesOfType(t).map((prop) => {
    ...
    const comment = docComment.length > 0 ? (0, import_typescript2.displayPartsToString)(docComment) : null;
    return {
      name: prop.name,
      data_type: dataType,
      primary_key: isKey,
...
var convertSourceFile = (sourceFile, checker) => {
  ...
  let exports2 = fileSymbol === void 0 ? [] : checker.getExportsOfModule(fileSymbol);
  exports2.forEach((exported) => {
    const declaration = exported.declarations[0];
    if ((0, import_typescript3.isInterfaceDeclaration)(declaration)) {
    ...
    if ((0, import_typescript3.isEnumDeclaration)(declaration)) {
      const t = checker.getTypeAtLocation(declaration);
...
//# sourceMappingURL=toDataModels.js.map